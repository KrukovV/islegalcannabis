...
- The local Source Review Workbench is read-only and must remain absent on production. An explicit source-review resolution must validate the entire schema-v5 registry, recompute every current payload/preimage/hash, bind the exact latest attempt ID, its signal-identity SHA-256 and the SHA-256 of the exact registry bytes, then append through an exclusive owned lock, staged durability write, exact-byte compare-and-swap and atomic rename. The resolution time must be a real current/past instant and may not be materially in the future. Stale, tampered, concurrent or cross-operation state fails before mutation; foreign locks and bytes are preserved. A resolved current operation leaves active Passport/Monitor queues but remains append-only history. No source-review outcome changes Legal Truth or Store Truth.
- The source-review registry schema must preserve every operation and attempt identifier. A current attempt retains the canonical signal payload, exact identity preimage and recomputable payload hash; a legacy attempt whose source fields were never recorded must say `LEGACY_SIGNAL_DETAILS_NOT_RECORDED_V1`, never invent reconstructed values. Workbench schema v3 returns the registry SHA-256 derived from the same bytes as its dossiers and an exact `operationId` lookup, and exposes only the retained current-source owner/applicability, fragment, current/effective, visual/screenshot, evidence-scope and confidence fields. A missing field stays `NOT_RECORDED`; Workbench must not infer it from another source or GEO.
