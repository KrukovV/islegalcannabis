...
- The local Source Review Workbench is read-only and must remain absent on production. The classifier and explicit resolver are concurrent writers to one registry and must share the same exclusive owned lock, staged durability write, intended-stage hash check, exact-preimage compare-and-swap and atomic rename. The classifier additionally binds the exact canonical reconciliation and canonical-GEO input bytes through commit. An explicit source-review resolution must validate the entire schema-v6 registry, recompute every current payload/preimage/attempt identity, bind the exact latest attempt ID, its signal-identity SHA-256 and the SHA-256 of the exact registry bytes. Every resolution validation and commit uses the same immutable official-domain and ownership-registry snapshots, regardless of evidence URL relation. The resolution time must be a real current/past instant and may not be materially in the future. Stale, tampered, concurrent or cross-operation state fails before mutation; foreign locks and bytes are preserved. A resolved current operation leaves active Passport/Monitor queues but remains append-only history. No source-review outcome changes Legal Truth or Store Truth.
- The source-review registry schema must preserve every operation and attempt identifier. A current attempt retains the canonical signal payload, exact identity preimage and recomputable payload hash; `attemptId` binds operation, signal identity and source checked-at. Latest selection never trusts mutable classifier time: it orders by recorded source checked-at and permits an equal-check tie only for the exact payload-equivalent V1-missing-owner/applicability to V2-corrected migration; any other equal-check ambiguity fails closed. A legacy attempt whose source fields were never recorded must say `LEGACY_SIGNAL_DETAILS_NOT_RECORDED_V1`, never invent reconstructed values. Canonical source aliases are normalized from camelCase and snake_case only when they agree; any owner/applicability conflict fails closed. Schema-v6 retains an incomplete `SOURCE_REVIEW_SIGNAL_V1` attempt and appends one corrected `SOURCE_REVIEW_SIGNAL_V2` attempt to the same unresolved operation; if the V1 operation was already resolved, the corrected V2 identity opens a new operation. A resolution covers only its exact reviewed signal. A later return to another historical signal, including the prior V1, opens a new deterministic operation rather than inheriting closure. A resolution must always bind that operation's semantic latest attempt; it cannot inherit an earlier V1 close after a V2 correction. Duplicate-open detection and close tokens use the same semantic ordering. Workbench schema v3 returns the registry SHA-256 derived from the same bytes as its dossiers and an exact `operationId` lookup, independently recomputes the exact current canonical V2 signal identity, and fails closed when the selected latest attempt is missing or stale. It exposes only the retained current-source owner/applicability, fragment, current/effective, visual/screenshot, evidence-scope and confidence fields. A missing field stays `NOT_RECORDED`; Workbench must not infer it from another source or GEO.
