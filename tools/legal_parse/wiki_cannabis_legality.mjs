...
const OUTPUT_PATH = path.join(ROOT, "data", "legal_raw", "wiki_legality.json");
const ISO_PATH = path.join(ROOT, "data", "iso3166", "iso3166-1.json");
const REPORT_PATH = path.join(
  ROOT,
  "Reports",
  "legal_parse",
  "wiki_missing_countries.json"
);
...
function parseStatus(value) {
  ...
  if (text.includes("medical")) return "legal";
  return "illegal";
}
...
for (const row of rows) {
  ...
  if (!iso2) {
    missing.push({ name: rawName, normalized });
    continue;
  }

  const recreationalText = stripTags(cells[1]);
  const medicalText = stripTags(cells[2]);
  const notesText = stripTags(cells[3] || "");

  wikiEntries[iso2] = {
    iso2,
    country: rawName,
    status_recreational: parseStatus(recreationalText),
    status_medical: parseStatus(medicalText),
    notes_wiki: notesText || null,
...
console.log(
  `OK wiki legality parsed (entries=${Object.keys(wikiEntries).length}, missing=${missing.length})`
);
