...
CURRENT_CMD="npm -w apps/web run test -- src/middleware.test.ts src/lib/wikiTruthNormalization.test.ts src/lib/wikiTruthAudit.test.ts src/lib/wikiTruthCounters.test.ts src/lib/officialSources/registry.test.ts src/lib/officialSources/officialLinkOwnership.test.ts src/lib/notesMerge/mergeNotes.test.ts src/lib/location/locationContext.test.ts src/lib/truth/mapTruthDataset.test.ts src/lib/ssotDiff/ssotDiffBuilder.test.ts src/lib/ssotDiff/ssotDiffEngine.test.ts src/lib/statusPairMatrix.test.ts src/lib/countryStatusDerivation.test.ts src/lib/countryPageStorage.test.ts src/app/api/check/contract.test.ts src/app/wiki-truth/page.test.ts src/app/changes/page.test.ts src/app/api/ssot/changes/route.test.ts"
TRUTH_TEST_LOG="${ROOT}/Reports/truth-tests.log"
rm -f "${TRUTH_TEST_LOG}" 2>/dev/null || true
if ! (cd "${ROOT}" && npm -w apps/web run test -- src/middleware.test.ts src/lib/wikiTruthNormalization.test.ts src/lib/wikiTruthAudit.test.ts src/lib/wikiTruthCounters.test.ts src/lib/officialSources/registry.test.ts src/lib/officialSources/officialLinkOwnership.test.ts src/lib/notesMerge/mergeNotes.test.ts src/lib/location/locationContext.test.ts src/lib/truth/mapTruthDataset.test.ts src/lib/ssotDiff/ssotDiffBuilder.test.ts src/lib/ssotDiff/ssotDiffEngine.test.ts src/lib/statusPairMatrix.test.ts src/lib/countryStatusDerivation.test.ts src/lib/countryPageStorage.test.ts src/app/api/check/contract.test.ts src/app/wiki-truth/page.test.ts src/app/changes/page.test.ts src/app/api/ssot/changes/route.test.ts) 2>&1 | tee -a "${TRUTH_TEST_LOG}"; then
