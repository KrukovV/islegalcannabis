...
- Final `pass_cycle` includes mandatory live production `/new-map` Method 1/2 Vercel bypass checks plus production payload/long-task checks, with PNG screenshots, timing measurements, and degradation thresholds from `data/baselines/prod_live_quality_baseline.json` and `data/baselines/new_map_payload_quality_baseline.json`.
