...
- The explicit close command is `node tools/review/resolve_source_review_operation.mjs --human-reviewed --operation-id=<id> --reviewer-id=<reviewer> --evidence-url=<https-official-evidence> --note=<review-note> --outcome=<CONFIRMED_CURRENT|SUPERSEDED> --resolved-at=<ISO-8601> --resulting-state=<reviewed-state> --resulting-reason=<reviewed-reason>`. A missing flag or provenance field fails closed; the command cannot change Legal Truth or Store Truth.
