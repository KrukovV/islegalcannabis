...
Every source/pending event opens one immutable review operation for one stable signal identity. Each later C1 check of that same signal appends a provenance-bound attempt and updates neither the operation nor its resolution; a genuinely different content/final-URL/access/hash/owner/applicability identity opens a new operation. C1 reachability, HTTP 304, byte equality, a redirect or a newer fetch state can never close semantic, effective-date, visual, applicability or source-change review. A resolution is appended only after an explicit human evidence review and must retain reviewer identity, the exact reviewed attempt and signal identity, a GEO-linked official evidence URL, note and real close time. The close operation is compare-and-swap guarded by the SHA-256 of the exact registry bytes, the exact latest attempt ID and its signal-identity SHA-256; stale registry state, another operation's attempt, a non-latest attempt or signal drift fails before mutation. `CONFIRMED_CURRENT` and `SUPERSEDED` remain source-review outcomes only; neither is a finding that the law is current and neither changes Legal Truth. An explicitly resolved current signal leaves the active Source/Pending queues but remains permanently visible in append-only history. Accounting or classifying an operation as `ACCESS_BLOCKED`, `APPLICABILITY_UNRESOLVED` or `CANONICAL_REVIEW_REQUIRED` leaves it open and must never be reported as human closure or `307/307` legal currency.
...
- `/truth-map/evidence-passport/review` and `/api/truth-map/b2b/source-review` are the local read-only Source Review Workbench. They classify each operation as current active, historical open or explicitly resolved, expose the exact latest-attempt provenance, cap an unfiltered response at 100 dossiers and accept only `geo`, `category` and `state=current|historical|resolved|open`. The API has no write handler and every non-local request returns `404`.
...
- Source Review Workbench accounts for the complete 307-GEO universe, keeps current and historical operations separate, and proves invalid/duplicate/empty filters fail closed. A controlled resolution fixture removes exactly one operation from the active Passport/Monitor projection while retaining its append-only history.
...
- `data/b2b_evidence/canonical_projection_ledger.json` contains the single honest 307-GEO baseline. `npm -w apps/web run evidence:snapshot -- --published-at=<ISO-8601>` appends only a different real canonical version, requires its real publication time and rejects an equal-version rewrite. Passport and Change Monitor read that append-only history; a later canonical-difference event uses the recorded current-version publication time rather than an inferred file or build date.
...
- `apps/web/src/truth-map/sourceReviewWorkbench.ts`, `/truth-map/evidence-passport/review` and `/api/truth-map/b2b/source-review` provide the bounded local read-only operations view; they never perform a resolution.
- `data/b2b_evidence/source_review_operations.json` is the append-only source-review operations, attempts and explicit human-resolution registry. `tools/review/build_source_review_operations.mjs` classifies every current source/pending/freshness signal, appends each real C1 attempt and retains earlier operations without auto-resolving them. `tools/review/resolve_source_review_operation.mjs` is the only normal close path and requires `--human-reviewed` plus reviewer, evidence, note, outcome and real close-time provenance; it binds the resolution to the exact latest attempt/signal and accepts only the retained source, its exact revalidated final URL or an official-domain/ownership-SSOT URL for the same GEO.
...
- `apps/web/src/truth-map/correctionRequest.ts` writes only untrusted append-only local receipts and separate assignment/decision/outcome events under `cache/b2b/`; the local review UI/API may approve a candidate only for manual canonical handoff, records no canonical operation id until that separate handoff actually occurs, and cannot promote any legal, Store, coordinate, leaf, map-position or ranking state.
