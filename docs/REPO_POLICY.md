...
- Large screenshot archives stay outside the repository. The repository keeps only bounded guard-required manifests/reports/metadata. Source-review raw artifacts are required and byte-validated while an attestation is written or migrated, then may remain external because SHA-256, byte length, magic-verified MIME and append-only chain state are committed; a locator is never evidence identity or an ordinary runtime dependency.
- The compact committed `data/b2b_evidence/source_review_evidence_v7_migration.json` receipt is permitted provenance metadata. It binds the exact bounded registry migration and no-Truth-change boundary; it must not embed raw screenshot bytes or become an alternate source-review/Legal/Store Truth registry.
- Historical matrix/visual coverage, C2, C3, machine-bound attestation, operation resolution and current-law currency are separate measurements. No `307/307` count in one layer may be reused as a completion claim for another.
...
- Local audit/UI worktree scope is explicit: narrowly scoped `.gitattributes`, `AGENTS.md`, branch-specific `vercel.json`, `CONTINUITY.md`, `Reports/**`, `data/wiki/**`, `data/wiki_cache/**`, `data/wiki_notes/**`, `data/official/**`, `data/reviews/**`, `data/store_truth/**`, `docs/**`, `tools/**`, and `apps/web/**`. `.gitattributes` may preserve authoritative source-snapshot bytes but may not suppress whitespace checks for product code. `AGENTS.md` is included only because it is the repository's versioned rules contract. Root `vercel.json` is allowed only for an explicit task-owned Git deployment guard and must have a matching executable test; it is not a general configuration bypass. `tools/ci-local.sh` passes only this named scope to the changed-path guard; other paths remain out of scope and require an explicit task-specific scope. A successful CI advances the existing baseline; a global scope bypass is forbidden.
...
- `pass_cycle` is a singleton transaction: it exports an active-run marker, rejects a child invocation with `PASS_CYCLE_REENTRY_BLOCKED=1`, and clears only its transient `.checkpoints` logs before each run. The final PTY summary is emitted from exact-run `Artifacts/runs/<RUN_ID>/ci-final.txt` only after all final guards, then copied to `Reports/ci-final.txt` for every value of `CI_WRITE_ROOT`; `Tools/commit_if_green.sh` consumes that Reports copy. `CI_WRITE_ROOT=1` additionally refreshes legacy repository-root `./ci-final.txt` only.
...
- Финальный `pass_cycle` обязан включать live production gates канонического `/` и отдельную проверку parameter-preserving permanent redirect `/new-map` → `/`: один Vercel root seed request для диагностики, PNG-скрины, `elapsed_ms`/`map_ready_ms`/transfer metrics, и деградационные baselines `data/baselines/prod_live_quality_baseline.json` + `data/baselines/new_map_payload_quality_baseline.json`. Cookie evidence is diagnostic only.
