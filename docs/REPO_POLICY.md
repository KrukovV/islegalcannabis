...
- Local audit/UI worktree scope is explicit: `AGENTS.md`, `CONTINUITY.md`, `Reports/**`, `data/wiki/**`, `data/wiki_cache/**`, `data/wiki_notes/**`, `data/official/**`, `data/reviews/**`, `docs/**`, `tools/**`, and `apps/web/**`. `AGENTS.md` is included only because it is the repository's versioned rules contract. `tools/ci-local.sh` passes only this named scope to the changed-path guard; other paths remain out of scope and require an explicit task-specific scope. A successful CI advances the existing baseline; a global scope bypass is forbidden.
