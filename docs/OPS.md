...
The shared UI guard first checks `127.0.0.1:3000`, the listener, recorded PID
and matching Next process. If HTTP is absent and every recorded owner is dead,
it may remove only the exact stale `apps/web/.next/dev/lock` file and stale PID
marker before restarting isLegal on `3000`. A live foreign or ambiguous owner
remains fail-closed; never delete lock directories recursively or start on
another port.
...
If a healthy existing dev server is detected at `http://127.0.0.1:3000/wiki-truth`, the guard prints:

```text
UI_ALREADY_RUNNING url=http://127.0.0.1:3000/wiki-truth
```

An exact isLegal-owned process may be restarted when necessary after PID,
command and working-directory verification. Never kill unrelated processes or
switch to another port automatically.
...
Do not rewrite an external authority as the target GEO to make the resolver pass. A non-307 `sourceOwnerGeo` is accepted only through the active `source_authority_owners` registry embedded in the already SHA-bound ownership SSOT and only for a matching source host. `appliesToGeos` remains canonical and the extension basis remains mandatory. The authority ID and its compatibility aliases are provenance only; they do not make a global source effective for country coverage. A missing owner, generic `UN`/`INTL`, archive, non-government or `UNCONFIRMED*` token remains an open metadata/review blocker.
