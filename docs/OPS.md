...
- `Reports/new-map-js-city/prod-js-city-gate-*.deep.chromium.png`
- `PROD_JS_CITY_METRIC` line in `Reports/ci-final.txt` with JS transfer, estimated unused JS, legacy-polyfill signals, country/city/deep label timing, road/landscape counts, deep opacity, and screenshot paths.
...
- `PROD_GPS_METRIC` line in `Reports/ci-final.txt` with stale-GPS refresh, GPS marker/center/recenter/persistence timings, UI GPS source/hint, hover result, ZoomIn city/village labels, ZoomOut rendered countries, first-party error counts, and screenshot paths.
