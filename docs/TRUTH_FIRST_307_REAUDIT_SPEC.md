...
### Current progress update: 2026-08-12 RE packet

This RE checkpoint supersedes the earlier VI count above without rewriting that historical record. The canonical ledger now has 241/307 structured independent packets, 273/307 normalized independent Truth Colors and 34 GEO without an independent color: `GREEN=91`, `YELLOW=101`, `RED=19`, `UNKNOWN=62`, `NO_INDEPENDENT_COLOR=34`. Current remaining IDs are `RU, SH, KN, LC, SM, SA, SN, SER, KAS, SL, SX, SB, SO, GS, SS, LK, SD, SR, SY, TJ, TZ, TH, TL, TG, TO, TT, TN, TM, TC, TV, US, EH, YE, ZM`.

RE retains in the one canonical ledger: the French Code de la santé publique Article L711-1 territorial bridge; current ANSM patient and professional cannabis-medicine pages; and the direct ANSM 24 June 2026 PUI workbook. The workbook records the exact `Liste PUI` heading `D3` and rows `C184:H186`, naming CH Ouest Réunion, CHU La Réunion GHSR and CHU Nord Réunion as Réunion facilities in the register of PUI trained and authorised to dispense medical cannabis. This directly corroborates local supply only; eligibility and the clinical route come independently from the current ANSM pages, while new enrolment remains closed. The one C1 run fetched seven shared canonical URLs once: Légifrance returned `403` and remains `ACCESS_BLOCKED` access metadata only; the current ANSM records returned `200` and are queued C2/C3 without a runner mutation of legal axes. RE is proposal-only `GREEN` under the general transitional operational-patient-access rule; strict visual acceptance remains partial under the active no-desktop-browser constraint. `APPLY_ALLOWED=false`, `PRODUCTION_TOUCHED=false`, `MAP_COLORS_CHANGED=false`, and `SSOT_CHANGED=false`.
