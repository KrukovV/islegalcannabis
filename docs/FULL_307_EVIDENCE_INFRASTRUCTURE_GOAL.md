...
Каждый source/pending event открывает immutable review case, ключом которого служат GEO, source URL, event kind, revalidation state и change reason; operation ID дополнительно связывает его начальную signal identity. Новая signal identity обычно открывает новую operation. Единственное bounded migration-исключение — incomplete V1 owner/applicability identity: schema-v6 сохраняет V1 attempt и добавляет ровно один corrected V2 attempt к той же unresolved operation; если V1 уже resolved, V2 identity открывает новую operation. Resolution покрывает только точно проверенную signal identity: возврат к другой исторической identity, включая прежнюю V1, открывает новую deterministic operation и не наследует закрытие. C1-доступность, HTTP 304, совпадение байтов, redirect или новый fetch-state не закрывают semantic, effective-date, visual, applicability или source-change review. Resolution добавляется только после явной человеческой evidence-review с reviewer identity, проверенной ссылкой, заметкой и реальной датой закрытия, которая не может быть существенно в будущем. Перед закрытием sole resolver проверяет весь schema-v6 registry и пересчитывает каждый current payload, preimage, attempt ID и hash. Latest attempt выбирается по связанному с attempt ID source checked-at, а не по изменяемому classifier `attemptedAt`; равный check-time разрешён только для точной payload-equivalent V1→V2 owner/applicability migration, иначе состояние неоднозначно и блокируется. Resolution всегда связывает semantic latest attempt. Builder и resolver используют один exclusive owned lock, staged durability write с повторной проверкой intended bytes, exact-preimage CAS и atomic rename; builder удерживает exact canonical reconciliation и canonical-GEO snapshots до commit, а resolver использует одни immutable official-domain/ownership snapshots для validation и любого commit независимо от evidence relation. Future-dated/stale/stage-tampered/concurrent/cross-operation state отклоняется, foreign lock/bytes сохраняются. Каждый новый attempt хранит полный канонический signal payload, его воспроизводимый hash и exact identity preimage; исторические поля, которых в старом событии не было, остаются явно `NOT_RECORDED`, а не восстанавливаются догадкой. Source-owner/applicability aliases из camelCase и snake_case нормализуются только при точном совпадении. `CONFIRMED_CURRENT` и `SUPERSEDED` остаются только исходами source-review, не подтверждают автоматически актуальность закона и не меняют Legal Truth. Закрытая текущая операция исчезает из active Source/Pending очереди, но навсегда остаётся в append-only history; последующая новая signal identity требует отдельного review.

Локальный read-only Source Review Workbench schema v3 показывает current active, historical open и explicitly resolved операции отдельно, раскрывает полную ordered attempt history и exact close tokens из тех же байтов registry, а для current source — только реально сохранённые owner/applicability, exact fragment, current/effective, visual/screenshot, evidence-scope и confidence поля. Он независимо пересчитывает exact current canonical V2 signal identity и fail-closed отклоняет отсутствующий или stale latest attempt. Отсутствующее значение остаётся `NOT_RECORDED`, без догадки по другому source/GEO. Workbench фильтрует по canonical GEO, категории, lifecycle и exact operation ID, не имеет write API и отсутствует на production.
