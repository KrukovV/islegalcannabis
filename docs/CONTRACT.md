...
- `/new-map` JS diagnostics must expose first-party script transfer, estimated unused JS, legacy-polyfill signals, initial/country/city/deep-zoom PNG screenshots, country/city/village label latency, road/landscape feature visibility, and the `legal-fill` deep-zoom opacity guard. Local/prod measurements use `tools/measure_new_map_js_city_perf.mjs`; final prod gating uses `data/baselines/new_map_js_city_quality_baseline.json`. Modern production builds must not ship Next's module polyfill bundle; legacy detection is limited to real polyfill-module patterns, not normal Baseline API calls.
...
- GPS button behavior: IP fallback may initialize the map, but browser GPS must override IP after permission, move the `Where I am` marker, persist the fresh GPS point, and center the camera on it. If a saved GPS point exists, any click may recenter it immediately for feedback, but the click must still request fresh browser geolocation so movement between sessions or during the session is detected and persisted.
- Final prod GPS gate must seed stale saved GPS, then verify fresh marker/center/recenter/persistence, `data-location-source="gps"`, non-IP GPS hint, desktop hover, ZoomIn to city/village labels, ZoomOut to country rendering, screenshots, zero first-party page errors, and zero first-party HTTP/failed resource errors.
...
- Final `pass_cycle` must also run the live production `/new-map` JS country/city/deep-label gate, write initial, country zoom, city zoom, and deep zoom PNG screenshots plus JSON under `Reports/new-map-js-city/`, and compare JS transfer, estimated unused JS, legacy-polyfill signals, rendered countries, label latency, road/landscape visibility, and deep opacity against `data/baselines/new_map_js_city_quality_baseline.json`.
- Final `pass_cycle` must also run the live production `/new-map` GPS/hover/zoom gate, write after-GPS, recenter, hover, ZoomIn, and ZoomOut PNG screenshots plus JSON under `Reports/new-map-gps/`, and require saved GPS to recenter immediately while still refreshing to a fresh browser GPS position.
