...
## Public map cold-start contract
- Canonical `/`, `/c/[code]` and the localhost-only `/new-map` legacy QA route must use one MapLibre implementation and one countries SSOT payload. Production `/new-map` permanently redirects to `/` and must not render a second map surface.
...
- Canonical root `/` cold start must not eagerly request optional country card index or US-state payloads. Card index may load for SEO/selected geo flows; US states may load after US-state selection or zoom threshold. Localhost `/new-map` must preserve the same behavior when used for explicit legacy QA.
...
- Public-map JS diagnostics must expose first-party script transfer, estimated unused JS, legacy-polyfill signals, initial/city PNG screenshots, and city-label latency after zoom. Local/prod measurements use the historically named `tools/measure_new_map_js_city_perf.mjs`; final production gating targets canonical `/` and uses `data/baselines/new_map_js_city_quality_baseline.json`. Modern production builds must not ship Next's module polyfill bundle; legacy detection is limited to real polyfill-module patterns, not normal Baseline API calls.
...
- The dotted in-place SEO panel is not an independent legal resolver. Across all 307 GEO, its badge, colour, title, law snapshot and current summary must use the selected final-reconciliation Truth Map card exactly; legacy country SEO data may supply only non-legal supporting navigation and must never introduce a competing current conclusion. With the panel open, the selected GEO keeps one visible `i` marker in the unobscured map area, and selecting another GEO keeps the panel available while opening that GEO's rich popup.
...
- `moveend` is the sole Store viewport scheduler for a completed pan or zoom. A second equivalent `zoomend` subscription is forbidden: it races the valid replacement request and can create a blank or flickering hand-off. The in-memory dataset cache is valid only while the source, record, eligibility and final-reconciliation file signatures all match; it reduces repeat parsing without changing the visibility gate or retaining a stale legal projection.
...
- Authority identity is not territorial applicability. A schema-v7 evidence record may retain a non-307 compatibility owner value only when it resolves to an active typed authority ID or explicit alias in the SHA-bound `source_authority_owners` section of `official_link_ownership`, its source host matches the authority's bounded official-domain set, every `appliesToGeos` target is canonical, the operation GEO is included, and a cross-owner or multi-GEO record carries a non-empty legal basis for extension. The authority registry never adds a GEO, changes country coverage or substitutes for applicability. Generic `UN`, `INTL`, `WEB_ARCHIVE`, `UNCONFIRMED*` and `source_owner_scope` values cannot satisfy owner identity.
...
- Final `pass_cycle` must also run the live production canonical `/` payload/long-task gate, write a PNG screenshot and JSON report under `Reports/new-map-payload/`, and compare total transfer, countries transfer, optional first-screen payloads, rendered countries, long tasks, and first-fill timing against `data/baselines/new_map_payload_quality_baseline.json`.
- Final `pass_cycle` must also run the live production canonical `/` JS country/city-label gate, write initial, country zoom, and city zoom PNG screenshots plus JSON under `Reports/new-map-js-city/`, and compare JS transfer, estimated unused JS, legacy-polyfill signals, rendered countries, country-label latency, and city-label latency against `data/baselines/new_map_js_city_quality_baseline.json`.
- Final `pass_cycle` must also run the live production canonical `/` GPS/hover/zoom gate, write after-GPS, recenter, hover, ZoomIn, and ZoomOut PNG screenshots plus JSON under `Reports/new-map-gps/`, and require stale saved GPS to refresh to a fresh browser GPS position.
