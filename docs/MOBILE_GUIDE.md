...
- The countries layer is loaded from the versioned immutable Brotli URL `/static/countries/countries.<hash>.json.br`.
- Slow-network production checks must confirm: map canvas visible, pan/zoom usable, AI/Social/DM controls absent, countries fill rendered, and country popup opens. The editable AI dock is verified only on permitted localhost QA/audit routes.
