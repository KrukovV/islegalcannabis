...
- A stability tag may be created only after `bash tools/pass_cycle.sh` passes and `Reports/ci-final.txt` contains `POST_CHECKS_OK=1`, `HUB_STAGE_REPORT_OK=1`, and `PROD_LIVE_OK=1`. Extended production gates must either pass when explicitly enabled with `PROD_EXTENDED_TAIL_GATES=1` or be skipped by the default production attempt budget.
