    ...
    <main className="container" style={{ paddingBlock: 24 }}>
      ...
      <section style={{ display: "grid", gap: 8, marginBottom: 20 }}>
        ...
        <div
          ...
          style={{ marginTop: 6, border: "1px solid #e5e7eb", borderRadius: 12, background: "#f8fafc", padding: 12, color: "#334155" }}
        >
          <div style={{ fontWeight: 600, marginBottom: 6 }}>Проверка актуальности страницы:</div>
          <div
          ...
          <div style={{ fontSize: 11, color: "#64748b" }}>
            Доступные данные в этой сборке: карта выключена, источник проверок — SSOT.
          </div>
        </div>
      </section>
      <section
        data-testid="wiki-truth-recent-changes"
        style={{ marginBottom: 20, border: "1px solid #e5e7eb", borderRadius: 12, background: "#fff", padding: 16, display: "grid", gap: 10 }}
      >
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <h2 style={{ margin: 0, fontSize: 18 }}>Последние изменения SSOT</h2>
          <div style={{ fontSize: 12, color: "#6b7280" }}>Кэш изменений сформирован: {diffCache.generated_at}</div>
        </div>
        ...
        <ul style={{ margin: 0, paddingLeft: 18, color: "#374151", fontSize: 14 }}>
          ...
          {!diffCache.last_24h.length ? <li>За последние 24 часа подтверждённых изменений нет.</li> : null}
        </ul>
      </section>
      <WikiTruthTable
        audit={audit}
        ...
        truthMapDisplay={pageModel.truthMapDisplay}
        storeAudit={pageModel.cannabisStoreAudit}
        goalAcceptance={pageModel.cannabisGoalAcceptance}
      />
    </main>
  );
}
