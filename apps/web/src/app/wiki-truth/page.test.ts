...
describe("/wiki-truth", () => {
  ...
  it("renders the complete protected cannabis-law matrix as the primary ordered audit", () => {
    ...
    expect(matrix).toContain(">34<");
    expect(matrix).toContain("Честно осталось серыми");
    expect(matrix).toContain(">5<");
  ...
  it("renders exactly 307 current-map versus official-law color rows with explanations", () => {
    ...
    expect((table.match(/data-reaudit-result="COLOR_RESOLVED"/g) || []).length).toBe(34);
    expect((table.match(/data-reaudit-result="HONEST_GREY_RETAINED"/g) || []).length).toBe(5);
    expect((table.match(/data-official-color="UNKNOWN"/g) || []).length).toBe(5);
