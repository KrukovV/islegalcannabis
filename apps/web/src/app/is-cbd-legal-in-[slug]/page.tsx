...
export function generateMetadata({
  ...
  const canonical = `/is-cbd-legal-in-${entry.slug}`;
  return {
    title: `Is ${EXTRA_LABEL} legal in ${entry.displayName}?`,
...
export default function CbdSeoPage({ params }: { params: { slug: string } }) {
  const entry = getSeoEntryBySlug(params.slug);
  if (!entry) notFound();
    ...
    <main className={styles.page}>
      <div className={styles.container}>
        <section className={styles.card}>
          ...
          <h1>{`Is ${EXTRA_LABEL} legal in ${entry.displayName}?`}</h1>
