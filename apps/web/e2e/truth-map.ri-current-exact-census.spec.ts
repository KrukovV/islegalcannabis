...
test("truth-map renders a current Rhode Island CCC leaf only after the exact Census gate", async ({ page }) => {
  test.setTimeout(60_000);
  await page.goto("/truth-map?qa=1&lat=41.838067732489&lng=-71.414404578734&zoom=13", { waitUntil: "domcontentloaded" });
  ...
  await page.waitForFunction(() => {
    ...
    return map?.queryRenderedFeatures({ layers: ["validated-cannabis-store-markers"] })
      .some((feature) => feature.properties?.store_id === "US-RI:LICENSE_LOCATION:mmp-cc-001:7716523ee536ce2d78acb405");
  }, { timeout: 30_000 });

  const store = await page.evaluate(() => {
    const map = window.__TRUTH_MAP_DEBUG__?.map;
    const feature = map?.queryRenderedFeatures({ layers: ["validated-cannabis-store-markers"] })
      .find((candidate) => candidate.properties?.store_id === "US-RI:LICENSE_LOCATION:mmp-cc-001:7716523ee536ce2d78acb405");
  ...
  await expect(page.getByTestId("store-popup")).toContainText("Thomas C. Slater Compassion Center");
  await expect(page.getByTestId("store-popup")).toContainText("License: MMP CC 001 · UNKNOWN_STATUS");
