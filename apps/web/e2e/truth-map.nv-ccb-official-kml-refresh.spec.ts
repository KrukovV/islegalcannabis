...
test("truth-map renders an exact current Nevada CCB official-map retail leaf", async ({ page }) => {
  test.setTimeout(60_000);
  await page.goto("/truth-map?qa=1&lat=36.1143045&lng=-115.1369195&zoom=15", { waitUntil: "domcontentloaded" });
  ...
  await page.waitForFunction(() => {
    ...
    return map?.queryRenderedFeatures({ layers: ["validated-cannabis-store-markers"] })
      .some((feature) => feature.properties?.store_id === "US-NV:LICENSE:43766627397168972639");
  }, { timeout: 30_000 });

  const store = await page.evaluate(() => {
    const map = window.__TRUTH_MAP_DEBUG__?.map;
    const feature = map?.queryRenderedFeatures({ layers: ["validated-cannabis-store-markers"] })
      .find((candidate) => candidate.properties?.store_id === "US-NV:LICENSE:43766627397168972639");
  ...
  await expect(page.getByTestId("store-popup")).toContainText("PISOS");
  await expect(page.getByTestId("store-popup")).toContainText("License: 43766627397168972639 · ACTIVE");
